(() => {
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .tns-monaco-search-toggle {
        width: 34px; height: 34px; border: 1px solid rgba(148,163,184,.35);
        border-radius: 9px; background: rgba(15,23,42,.94); color: #dbeafe;
        display: grid; place-items: center; padding: 0; cursor: pointer;
        box-shadow: 0 5px 16px rgba(0,0,0,.28);
      }
      .tns-monaco-search-toggle:hover,
      .tns-monaco-search-toggle.active { background: #1d4f7a; border-color: #65a9e8; }
      .tns-monaco-search-toggle svg {
        width: 18px; height: 18px; stroke: currentColor; fill: none; stroke-width: 2;
      }
      .tns-monaco-search-panel {
        position: absolute; z-index: 34; top: 6px; right: 49px;
        width: min(330px, calc(100% - 66px)); max-height: calc(100% - 12px);
        overflow: hidden; border: 1px solid rgba(71,94,128,.7); border-radius: 10px;
        background: rgba(9,18,34,.98); color: #d7e5f7;
      ...
      .tns-monaco-search-input-wrap {
        ...
        background:#111d31; border:1px solid #39506f; border-radius:6px; padding:0 4px 0 8px;
      }
      ...
      .tns-monaco-search-result {
        width:100%; border:0; border-radius:5px; background:transparent; color:#cbd9e9;
        text-align:left; display:grid; grid-template-columns:38px minmax(0,1fr); gap:6px;
        padding:5px 7px; cursor:pointer; font:11px/1.35 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;
      }
      ...
      @media (pointer: coarse) {
        ...
        .monaco-editor textarea.inputarea { -webkit-touch-callout: default; }
      }
    `;
  ...
  function focusLuaMonaco(raw) {
    const input = raw?.closest(".lua-editor-wrap")?.querySelector(".monaco-editor textarea.inputarea");
    if (input) input.focus();
    else raw?.focus?.();
  }

  function emitLuaInput(raw) {
  ...
  function replaceLuaSelection(raw, replacement) {
    if (!raw) return;
  ...
  function installLuaMobileEditActions() {
    ...
    panel.append(makeLuaEditButton("lua-copy-selection", "Copiar selección", async () => {
      const raw = luaEditorField();
      if (!raw) return;
      const start = Number(raw.selectionStart || 0);
      const end = Number(raw.selectionEnd || 0);
      if (end <= start) {
        luaToolStatus("No hay texto seleccionado.");
        return;
      }
      await writeClipboard(String(raw.value || "").slice(start, end));
    ...
    panel.append(makeLuaEditButton("lua-paste-selection", "Pegar", async () => {
      ...
      try {
        ...
        const manual = window.prompt("Safari no permitió leer el portapapeles. Pega el texto aquí:", "");
        if (manual === null) return;
        text = manual;
      }
      replaceLuaSelection(raw, text);
  ...
  const observer = new MutationObserver(() => installLuaMobileEditActions());
  const startObserver = () => {

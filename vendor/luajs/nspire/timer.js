...
// TI-Nspire document/CAS bridge. The real TI Lua path is:
// Lua -> math.eval("func(...)") -> Func stored in Problem XML <sym>.
(function installTnsDocumentMathBridge() {
  ...
  function isTiPreview() {
    try {
      ...
      if (!log) return true;
      return /TI[- ]Nspire code detected|Codigo TI[- ]Nspire detectado|Code TI[- ]Nspire/i.test(String(log.textContent || ""));
  ...
  schedule(function () {
    ...
    function splitArgs(text) {
      ...
      if(cur.trim() || text.trim()==="") parts.push(cur.trim());
      return parts;
    }

    var stagedDefs = Object.create(null);
    function localName(node) { return String(node && node.localName || node && node.nodeName || "").replace(/^.*:/,""); }
    function addDefinition(name,params,body) {
      name=String(name||"").trim(); params=String(params||"").trim(); body=String(body||"").replace(/\r\n?/g,"\n");
      if(name && /^\s*Func\b/i.test(body)) stagedDefs[name]={params:params,body:body};
    }
    function parseRawXml(raw) {
      var out={name:"",params:"",body:""};
      if(!raw) return out;
      try {
        var doc=new DOMParser().parseFromString(String(raw),"application/xml");
        var root=doc.documentElement;
        var all=root ? root.children : [];
        for(var i=0;i<all.length;i++) {
          var n=localName(all[i]);
          if(n==="n") out.name=all[i].textContent||"";
          else if(n==="p") out.params=all[i].textContent||"";
          else if(n==="v") out.body=all[i].textContent||"";
        }
      } catch (_error) {}
      return out;
    }
    function collectStagedFuncs() {
      var candidates=[];
      try { if (typeof xmlDoctor !== "undefined" && xmlDoctor && Array.isArray(xmlDoctor.candidates)) candidates=xmlDoctor.candidates; } catch (_error) {}
      ...
      for(var i=0;i<candidates.length;i++) {
        var item=candidates[i]; if(!item) continue;
        var type=String(item.document_type || item.type || item.kind || "");
        var body=String(item.code || item.content || "");
        if(type!=="Func" && !/^\s*Func\b/i.test(body)) continue;
        var name=String(item.program_name || item.name || "").trim();
        var params=String(item.parameters || item.params || (item.detail && item.detail.parameters) || "").trim();
        if((!name || !body) && item.raw_xml) {
          var parsed=parseRawXml(item.raw_xml);
          if(!name) name=parsed.name;
          if(!params) params=parsed.params;
          if(!body) body=parsed.body;
        }
        addDefinition(name,params,body);
      }
      try {
        var current=(typeof xmlDoctor!=="undefined"&&xmlDoctor)?xmlDoctor.current:null;
        if(current) {
          var currentBody=String((document.querySelector("#xml-code")||{}).value || current.code || current.content || "");
          var currentType=String(current.document_type || current.type || "");
          if(currentType==="Func" || /^\s*Func\b/i.test(currentBody)) addDefinition(current.program_name || current.name,current.parameters || current.params,currentBody);
        }
      } catch (_error) {}
    }
    collectStagedFuncs();
    ...
    function splitStatements(body) {
      ...
      for(var i=0;i<text.length;i++) {
        ...
        if(quote){cur+=ch;if(ch==="\\"){if(i+1<text.length)cur+=text.charAt(++i);}else if(ch===quote)quote="";continue;}
        if(ch==='"'||ch==="'"){quote=ch;cur+=ch;continue;}
        if(ch==="("||ch==="["||ch==="{")depth++; else if(ch===")"||ch==="]"||ch==="}")depth=Math.max(0,depth-1);
        if((ch==="\n" || (ch===":" && text.charAt(i+1)!=="=")) && depth===0) { if(cur.trim()) out.push(cur.trim()); cur=""; }
        else cur+=ch;
      }
    ...
    function evalFunc(def,args,depth) {
      ...
      for(var j=0;j<statements.length;j++) {
        ...
        if(ret) return ret[1] ? evalExpression(ret[1],scope,depth) : last;
        ...
        var arrow=/^(.+?)(?:->|→)\s*([A-Za-z_][A-Za-z0-9_]*)$/.exec(s);
        if(arrow){scope[arrow[2]]=evalExpression(arrow[1],scope,depth);last=scope[arrow[2]];continue;}
    ...
    window.__tnsTiFuncCount=Object.keys(stagedDefs).length;
  });
})();

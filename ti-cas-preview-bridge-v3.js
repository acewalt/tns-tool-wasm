(() => {
  ...
  const BRIDGE_VERSION = "2026-09-03-giac-preview-v3";
  // Single-file Emscripten build with the WASM payload embedded as base64.
  // This avoids the cross-origin .wasm fetch/MIME problem seen on GitHub Pages.
  const GIAC_EMBEDDED_SCRIPT_URL = "https://cdn.jsdelivr.net/gh/OpenMath-eu/onlymath-engine@7d3ccf044bd7fdd9468299ba03b8ff01b1981d39/giacwasm.js";
  ...
  async function loadBackend(scriptUrl, wasmUrl, backendName, timeoutMs) {
    ...
    const module = {
      ...
      printErr: () => {},
      onAbort(reason) {
        runtimeReject(new Error(`${backendName} abortó: ${String(reason || "sin detalle")}`));
      },
    ...
    try {
      await Promise.race([
        Promise.all([scriptLoaded, runtimeReady]),
        timeoutPromise(timeoutMs, `${backendName} no estuvo listo después de ${Math.round(timeoutMs / 1000)} s.`)
      ]);
    } catch (error) {
      removeFrame(frame);
      throw error;
    ...
    const runtimeModule = win.Module || module;
    const candidate = exportedCaseval(runtimeModule);
    if (!candidate) {
      removeFrame(frame);
      throw new Error(`${backendName} cargó pero no expuso caseval().`);
    }

    return { frame, caseval: candidate, backend: backendName };
  }
  ...
  async function initializeGiac(timeoutMs = DEFAULT_LOAD_TIMEOUT_MS) {
    ...
    state.loadPromise = (async () => {
      const perBackend = Math.min(BACKEND_TIMEOUT_MS, Math.max(8000, Math.floor(timeoutMs / 3)));
      const failures = [];
      const candidates = [
        [GIAC_EMBEDDED_SCRIPT_URL, null, "Giac WASM CDN"],
        [GIAC_WASM_SCRIPT_URL, GIAC_WASM_URL, "Giac WASM upstream"],
        [GIAC_ASM_SCRIPT_URL, null, "Giac asm.js upstream"]
      ];

      for (const [scriptUrl, wasmUrl, backendName] of candidates) {
        try {
          const loaded = await loadBackend(scriptUrl, wasmUrl, backendName, perBackend);
          ...
          failures.push(`${backendName}: ${String(error?.message || error)}`);
        }
      }
...
`;
  ...
  if (typeof oldCreateLuaJsPreviewRuntime !== "function") {
    console.warn("TNS CAS bridge v3: createLuaJsPreviewRuntime no está disponible.");
    return;
  }
  ...
  window.createLuaJsPreviewRuntime = async function (code, ctx, canvas, logEl, symbols = {}) {
    ...
    if (needsCas) {
      appendLog(logEl, "CAS Preview v3: cargando Giac/Xcas para math.eval/math.evalStr...");
      const ready = await initializeGiac(DEFAULT_LOAD_TIMEOUT_MS);
      if (ready) {
        appendLog(logEl, `CAS Preview v3: ${state.backend || "Giac"} listo.`);
        if (state.error) appendLog(logEl, `CAS Preview v3: backends anteriores fallaron (${state.error}).`);
      } else {
        appendLog(logEl, `CAS Preview v3: Giac no disponible; se usará el evaluador local (${state.error || "error desconocido"}).`);
      }
    }
